...
exports.default = {
    ...
    create: async (req, res) => {
        const { productora_id, contacto_id, direccion, lat, lng, hora_entrada, hora_salida, fecha, valor_por_persona, valor_hora_extra, cantidad_personas } = req.body;
        // Basic validation
    ...
    reprogramar: async (req, res) => {
        ...
        try {
            await db_1.default.query('UPDATE eventos SET fecha = $1, hora_entrada = $2, hora_salida = $3 WHERE id = $4', [fecha, hora_entrada, hora_salida, id]);
    ...
    updateStatus: async (req, res) => {
        ...
        try {
            if (estado === 'iniciado') {
                const { rows } = await db_1.default.query('SELECT direccion, hora_entrada FROM eventos WHERE id = $1', [id]);
                const evento = rows[0];
            ...
            await db_1.default.query('UPDATE eventos SET estado = $1 WHERE id = $2', [estado, id]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    updateExtras: async (req, res) => {
        const { id } = req.params;
        const { personas_extras, horas_extras } = req.body;
        try {
            await db_1.default.query('UPDATE eventos SET personas_extras = $1, horas_extras = $2 WHERE id = $3', [personas_extras || 0, horas_extras || 0, id]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    updatePagado: async (req, res) => {
        const { id } = req.params;
        const { pagado } = req.body;
        try {
            await db_1.default.query('UPDATE eventos SET pagado = $1 WHERE id = $2', [pagado ? true : false, id]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    updatePagos: async (req, res) => {
        const { id } = req.params;
        const { abono_monto, abono_fecha, pago_final_monto, pago_final_fecha, factura_base, factura_extras, pago_extras_monto, pago_extras_fecha } = req.body;
        try {
            await db_1.default.query(`
        UPDATE eventos SET 
